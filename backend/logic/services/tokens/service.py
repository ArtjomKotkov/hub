...
class TokensService:
    ...
    def _generate_tokens(self, user: User) -> tuple[str, str]:
        ...
        auth_token = encode(json.loads(auth_token_payload.json()), Settings.APP_SECRET, algorithm=self._encode_algorithm)
        refresh_token = encode(json.loads(refresh_token_payload.json()), Settings.APP_SECRET, algorithm=self._encode_algorithm)
